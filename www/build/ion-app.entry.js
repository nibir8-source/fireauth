    ...
    if (Build.isBrowser) {
      rIC(async () => {
        ...
        if (!config.getBoolean('_testing')) {
          import('./tap-click-53989703.js').then(module => module.startTapClick(config));
        }
        if (config.getBoolean('statusTap', isHybrid)) {
          import('./status-tap-b0ce34b2.js').then(module => module.startStatusTap());
        }
        if (config.getBoolean('inputShims', needInputShims())) {
          import('./input-shims-848ec1cd.js').then(module => module.startInputShims(config));
        }
